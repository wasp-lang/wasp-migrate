...
async function migrate(): Promise<void> {
...

  {
    ...
    if (waspDepsStrMatches) {
      ...
      // Ignoring that waspDepsStr.match(...) might be `null`
      const deps = waspDepsStr.match(/\(\s*"[^"]+"\s*,\s*"[^"]+"\s*\)/g)!.map((depStr) => {
        // Ignoring that depStr.match(...) might be `null`
...
function assertWaspVersion(): void {
...
async function printWarning(): Promise<void> {
 console.log(
